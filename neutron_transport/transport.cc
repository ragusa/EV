...
class TransportProblem {
   ...
   private:
      ...
      Vector<double> max_viscosity_domain;
      Vector<double> entropy_viscosity_domain;
...
void TransportProblem<dim>::assemble_system() {
   ...
   // resize the viscosity vectors
   max_viscosity_domain.reinit(triangulation.n_active_cells());
   entropy_viscosity_domain.reinit(triangulation.n_active_cells());

   // Gauss quadrature formula
   QGauss<dim> quadrature_formula(degree + 1);

   // quadrature formula for faces, needed for FE face values
   ...
   std::vector<double> total_cross_section_values(n_q_points);
   // total source values for an energy group and direction at each
   //  quadrature point on cell
   std::vector<double> total_source_values(n_q_points);
   ...
   for (unsigned int g = 0; g < n_energy_groups; ++g) {
      ...
      for (unsigned int d = 0; d < n_directions; ++d) {
         ...
         // loop over cells
         unsigned int i_cell = 0; // cell index for viscosity vectors
         for (cell = dof_handler.begin_active(); cell != endc; ++cell) {
            ...
            double max_viscosity = parameters.max_viscosity_coefficient * h;
            max_viscosity_domain[i_cell] = max_viscosity;
            switch (parameters.viscosity_type) {
               case 0: {
                  break;
               } case 1: {
                  viscosity = max_viscosity;
                  break;
               } case 2: {
                  if (nonlinear_iteration != 0) {
                     ...
                     std::vector<double> entropy_deviation_values(n_q_points,0.0);
                     for (unsigned int q = 0; q < n_q_points; ++q)
                     ...
                     double entropy_viscosity = parameters.entropy_viscosity_coefficient *
                           h * h * max_entropy_residual / max_entropy_deviation_domain;
                     //if (entropy_viscosity != entropy_viscosity) std::cout << "NaN!" << std::endl;
                     entropy_viscosity_domain[i_cell] = entropy_viscosity;

                     // determine viscosity: minimum of first-order viscosity and entropy viscosity
                     viscosity = std::min(max_viscosity, entropy_viscosity);
                  } else
                     viscosity = max_viscosity;
               ...
               for (unsigned int i = 0; i < dofs_per_cell; ++i) {
                  for (unsigned int j = 0; j < dofs_per_cell; ++j) {
                     cell_matrix(i, j) += (
                        ...
                        // Jacobian of transformation to reference cell
                        ) * fe_values.JxW(q);
                  } // end j
   ...
   // eliminate constraints due to hanging nodes
      ...
      for (cell = dof_handler.begin_active(); cell != endc; ++cell) {
         ...
         for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
            ...
            if (cell->face(face)->at_boundary()) {
               ...
               if (fe_face_values.normal_vector(0) * transport_directions[d]
                     < small) {
...
void TransportProblem<dim>::run() {
   ...
   for (unsigned int cycle = 0; cycle < parameters.n_refinement_cycles;
      ...
      nonlinear_iteration = 0;
      // if using entropy viscosity, begin nonlinear iteration
   ...
   if (parameters.viscosity_type == 2) {
      ...
      visc_out.add_data_vector(max_viscosity_domain,"Max_Viscosity",DataOut<dim>::type_cell_data);
      visc_out.add_data_vector(entropy_viscosity_domain,"Entropy_Viscosity",DataOut<dim>::type_cell_data);
   ...
   if (parameters.exact_solution_id != 0) {
      ...
      convergence_table
      .evaluate_convergence_rates("L2 error", ConvergenceTable::reduction_rate_log2);
