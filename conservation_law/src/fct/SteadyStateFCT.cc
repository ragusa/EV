 ...
 */
template <int dim>
bool SteadyStateFCT<dim>::check_bounds(const Vector<double> & new_solution) const
{
  // loop over filters
 ...
 */
template <int dim>
Vector<double> SteadyStateFCT<dim>::get_lower_solution_bound() const
{
 ...
 */
template <int dim>
Vector<double> SteadyStateFCT<dim>::get_upper_solution_bound() const
{
  // make sure there is only one filter so there is no amibuity of which
  // bounds to output
  Assert(this->n_filters == 1, ExcNotImplemented());
