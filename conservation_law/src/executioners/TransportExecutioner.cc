    ...
    postprocessor(&postprocessor_)
{
  ...
  inviscid_ss_flux.reinit(n_dofs);
...
/**
 * \brief Computes the reaction vector.
 */
template <int dim>
void TransportExecutioner<dim>::compute_reaction_vector()
{
  reaction_vector = 0;
  ...
  {
    // initialize local vector to zer
    cell_vector = 0;
    ...
    for (unsigned int q = 0; q < n_q_points_cell; ++q)
    {
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        cell_vector(i) += transport_speed * fe_values[flux].value(i, q) *
          total_cross_section_values[q] * fe_values.JxW(q);
      } // end i
    }   // end q

    // aggregate local residual into global residual
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
      reaction_vector(local_dof_indices[i]) += cell_vector(i);
  } // end cell
}
