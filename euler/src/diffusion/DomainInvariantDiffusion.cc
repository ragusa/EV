...
{
  // reset max wave speed domain for checks
  max_wave_speed->reset_max_wave_speed_domain();
std::cout<<"computing diffusion matrix"<<n_dofs<<std::endl;
  ...
  for (unsigned int i = 0; i < n_dofs_scalar; ++i)
  {
    ...
    for (; it[0] != it_end; ++it_norm)
    {
      ...
      if (diffusion_matrix(i0, j0) != 0.0)
      {
        ...
        // invariant domain paper, lambda_max(i,j) = lambda_max(j,i), so there is
  ...
  // check entries
  double lambda_max = max_wave_speed->get_max_wave_speed_domain();
  for (unsigned int i = 0; i < n_dofs; ++i)
    std::cout << i << " " << diffusion_matrix(i,i) << " " << lambda_max << std::endl;
}
 ...
 */
template <int dim>
void DomainInvariantDiffusion<dim>::compute_gradients_and_normals()
{
  // reset matrices
  for (unsigned int d = 0; d < dim; ++d)
    gradients[d] = 0;
