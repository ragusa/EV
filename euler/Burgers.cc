 ...
 *  \param cell_residual residual contribution for the current cell, to be
 * aggregated into the global residual.
 */
/*
template <int dim>
...
burgers_parameters.first_order_viscosity_coef * cell_diameter * max_velocity;
               for (unsigned int q = 0; q < this->n_q_points_face; ++q)
...
velocity);
      ...
      for (unsigned int q = 0; q < this->n_q_points_cell; ++q)
         for (unsigned int i = 0; i < this->dofs_per_cell; ++i)
         {
            for (unsigned int j = 0; j < this->dofs_per_cell; ++j)
            {
               cell_matrix(i,j) += fe_values[velocity_extractor].gradient(i,q) *
ones_vector
                  * velocity[q] * fe_values[velocity_extractor].value(j,q) *
fe_values.JxW(q);
            }
         }
