...
%      \begin{equation}
%         \pd{u}{t} + \nabla\cdot\mathbf{f}(u) + \reactioncoef u\xt = q\xt
%      \end{equation}
...
%            \mathbf{f}'(u)\cdot\nabla\testfunction_j(\x) +
%            \reactioncoef(\x)\testfunction_j(\x)\right)\testfunction_i(\x) d\x
%      \end{equation}
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NOTE: I should ask Dr. Guermond's permission before using the figures in this
% slide.
%
%\begin{frame}
%\frametitle{Results}
...
%      \includegraphics[height=0.5\textheight]{./figures/Gal-FCT_rotation.png}
%      \caption{Galerkin-FCT}
%   \end{subfigure}
%   \begin{subfigure}{0.4\textwidth}
%      \includegraphics[height=0.5\textheight]{./figures/EV-FCT_rotation.png}
%      \caption{EV-FCT}
%   \end{subfigure}
%\end{figure}
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Definitions for the Shallow Water Equations}

\begin{itemize}
  \item $\height\xt \equiv \int\density\xt dz$ is referred to as ``height''.
  \item $\heightmomentum\xt \equiv \height\xt\velocity\xt$ is referred to as ``discharge''.
  \item $\gravity$ is the acceleration due to gravity.
  \item $\bathymetry(\x)$ is the bottom topography profile, or ``bathymetry''. For a
    flat bottom, $\bathymetry(\x)=0$.
\end{itemize}
...
\begin{itemize}
  ...
  \item Substituting the approximate solution
    ...
    and interpolating the flux function at the nodes,
    $\consfluxvector(\approximatevectorsolution)\rightarrow
      \sum_j\consfluxinterpolant_j(t)\testfunction_j(\x)$, and testing with
    $\testfunction_i(\x)$ gives
    \begin{equation}
      \sum_j\consistentmassentry\solutionvector_j(t)
    ...
    \end{equation}
  \item Expressing this as a linear system with forward Euler (FE) time
    discretization,
    \begin{equation}
      \consistentmassmatrix\frac{\solutionvector_j^{n+1}
        - \solutionvector_j^{n}}{\dt}
        + \gradientmatrix\cdot\consfluxinterpolant^n
        = \ssrhs^n \eqp
    \end{equation}
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Domain-Invariance}

\begin{itemize}
  \item For the systems case, discrete maximum principles no longer apply;
    \hlorange{domain-invariance} becomes the desired property.
  \item
\end{itemize}
