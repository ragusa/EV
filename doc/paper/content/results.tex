...
$21.94^\circ$ with respect to the x-axis. \tcr{what are $\Omega_x$ and $\Omega_y$ here?}
...
keep the solution within some physical bounds \tcr{'some' sounds vague. why some???}, but one can see from
...
as expected \tcr{justify 'as expected'}; the addition of the entropy-based artificial viscosity
...
superior due to less artificial diffusion. \tcr{do you show a latter situation?}

\tcr{is the blue color in this figure always the same numerical value? i do not think so. maybe colorbars should be added for completeness. i can imagine a reviewer complaining about this}
\begin{figure}[ht]
   \centering
...
%-------------------------------------------------------------------------------
\tcr{I understand the ail in the FCT column but why are there no numbers in the last row
for EV?}
