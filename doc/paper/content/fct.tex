...
high-order scheme: solving Equations~\eqref{eq:antidiffusion_source_ss} through
\eqref{eq:antidiffusion_source_theta} \tcr{these eqref do not work} still produces the high-order solution.
...
following goal: maximize the limiting coefficients such that the \tcr{some} imposed
...
\end{equation}
Furthermore, the antidiffusion bounds $\hat{p}_i^\pm$, which depend on these
...
\tcr{feels like this paragraph needs to be rephrased and simplified}
...
\tcr{I understand the details but may be missing the larger picture of the last paragraph. Is
this in the weeds and could be removed or is this very important, in which case it needs
to convey that importance more}
