...
void TransportProblem<dim>::process_problem_ID()
{
  switch (parameters.problem_id)
  {
    ...
    { // skew void-to-absorber
      ...
      { 0.5 };
      const std::vector<double> region_sources =
      { 0.0, 0.0 };
      const std::vector<double> region_sigmas =
      { 0.0, 10.0 };
...
void TransportProblem<dim>::run()
{
  ...
  {
    case TransportParameters<dim>::SS :
    {
      timedisc_string = "SS";
      break;
    }
    case TransportParameters<dim>::FE :
    {
      timedisc_string = "FE";
      break;
    }
    case TransportParameters<dim>::CN :
    {
      timedisc_string = "CN";
      break;
    }
    case TransportParameters<dim>::BE :
    {
      timedisc_string = "BE";
      break;
    }
    case TransportParameters<dim>::SSP2 :
    {
      timedisc_string = "SSPRK22";
      break;
    }
    case TransportParameters<dim>::SSP3 :
    {
  ...
  PostProcessor<dim> postprocessor(parameters.output_meshes,
      parameters.output_exact_solution, parameters.save_convergence_results,
      has_exact_solution, exact_solution_function, parameters.end_time,
      dt_nominal, !is_time_dependent, parameters.refinement_mode,
      final_refinement_level, parameters.exact_solution_refinement_level, fe,
      output_dir, appendage_ss.str(), filename_exact_ss.str(), cell_quadrature);

  // create refinement handler object
  RefinementHandler<dim> refinement_handler(triangulation, n_cells, dt_nominal,
      parameters.refinement_mode, parameters.use_adaptive_refinement,
  ...
  for (unsigned int cycle = 0; cycle < parameters.n_refinement_cycles; ++cycle)
  {
    ...
    LinearSolver<dim> linear_solver(parameters.linear_solver_option, constraints,
        dof_handler, incoming_function);

    // create low-order viscosity
    LowOrderViscosity<dim> low_order_viscosity(n_cells, dofs_per_cell,
        dof_handler, constraints, inviscid_ss_matrix, low_order_diffusion_matrix,
        low_order_ss_matrix);

    // create entropy viscosity
    EntropyViscosity<dim> EV(fe, n_cells, dof_handler, constraints,
        cell_quadrature, face_quadrature, transport_direction,
        cross_section_function, source_function, parameters.entropy_string,
        parameters.entropy_derivative_string,
        parameters.entropy_residual_coefficient, parameters.jump_coefficient,
        domain_volume, parameters.EV_time_discretization, low_order_viscosity,
        inviscid_ss_matrix, high_order_diffusion_matrix, high_order_ss_matrix);
...
double TransportProblem<dim>::enforce_CFL_condition(double & dt)
{
  ...
  for (unsigned int i = 0; i < n_dofs; ++i)
    max_speed_dx = std::max(max_speed_dx,
        std::abs(low_order_ss_matrix(i, i)) / lumped_mass_matrix(i, i));
