  ...
  for (; cell != endc; ++cell, ++i_cell)
  {
    ...
    for (unsigned int i = 0; i < this->dofs_per_cell; ++i)
      for (unsigned int j = 0; j < this->dofs_per_cell; ++j)
        if (i != j)
          low_order_viscosity_cell = std::max(
            low_order_viscosity_cell,
            std::max(
              0.0, inviscid_matrix(local_dof_indices[i], local_dof_indices[j])) /
...
dof_handler->begin_active(),
   ...
   for (unsigned int i = 0; i < n_dofs; ++i)
   {
      ...
      solution_max(i) = solution_max(i)*(1.0 -
dt/(*lumped_mass_matrix)(i,i)*row_sum)
         + dt/(*lumped_mass_matrix)(i,i)*ss_rhs(i);
      solution_min(i) = solution_min(i)*(1.0 -
dt/(*lumped_mass_matrix)(i,i)*row_sum)
         + dt/(*lumped_mass_matrix)(i,i)*ss_rhs(i);
   }
}
...
/*
template<int dim>
void LowOrderViscosity<dim>::output_bounds(const PostProcessor<dim>
&postprocessor) const
{
